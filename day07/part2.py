...
def main():
  ...
  for [hand, bid] in hands:
    ranked_hands.append({"type": get_type_rank(hand), "hand": resort_hand(hand), "bid": bid})
...
def resort_hand(hand):
    def custom_sort(char):
      order = ["A", "K", "Q", "T", "9", "8", "7", "6", "5", "4", "3", "2", "J"]
      return order.index(char)

    return ''.join(sorted(hand, key=custom_sort))
...
def full_house(hand):
  hand_dict = make_hand_dict(hand)
  if hand_dict[0] == 3 and hand_dict[1] == 2:
    print(hand)
